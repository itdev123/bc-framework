...
final class uac_roles_groups implements \BriskCoder\Package\Module\bcUAC\Libs\RBAC\Model\i_uac_roles_groups
{
    ...
    public function __construct( $CALLER )
    {
        ...
        bcCache::useNamespace( $tbl->tbl_uac_roles_groups );
        bcCache::useNamespace( $tbl->tbl_uac_roles_groups_text );
    ...
    {
        $tbl = bcUAC::rbac()->configModel();
        bcCache::deleteNamespace( $tbl->tbl_uac_roles_groups, TRUE );
        bcCache::deleteNamespace( $tbl->tbl_uac_roles_groups_text, TRUE );
        bcCache::deleteNamespace( $tbl->tbl_uac_roles_groups . '_' . $tbl->tbl_uac_roles_groups_text, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->delete( $tbl->tbl_uac_roles_groups );
    ...
    public function update( $_columns, $_filters, $_order_by, $offset )
    {
        ...
        bcCache::deleteNamespace( $tbl->tbl_uac_roles_groups . '_' . $tbl->tbl_uac_roles_groups_text, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->update( $tbl->tbl_uac_roles_groups, $_columns );
        bcDB::mapper()->where( $_filters );
        bcDB::mapper()->orderBy( $_order_by );
        bcDB::mapper()->limit( $offset );
        return bcDB::mapper()->exec();
    ...
    public function write( $_columns, $last_id )
    {
        ...
        bcCache::deleteNamespace( $tbl->tbl_uac_roles_groups . '_' . $tbl->tbl_uac_roles_groups_text, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->insert( $tbl->tbl_uac_roles_groups, $_columns );
    ...
    {
        $cls = 'BriskCoder\Package\Module\bcUAC\Libs\RBAC\Model\\' . bcDB::resource( 'RDBMS' ) . '\uac_roles_groups\uac_roles_groups_text';
