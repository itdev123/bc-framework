...
final class uac_permissions implements \BriskCoder\Package\Module\bcUAC\Libs\RBAC\Model\i_uac_permissions
{
    ...
    public function __construct( $CALLER )
    {
        ...
        bcCache::useNamespace( $tbl->tbl_uac_permissions );
    }

    /**
     * uac_permissions TABLE SQL ALTER OPERATION
    ...
    {
        $tbl = bcUAC::rbac()->configModel();
        bcCache::deleteNamespace( $tbl->tbl_uac_permissions, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->alter( $tbl->tbl_uac_permissions, $type, $_columns );
        return bcDB::mapper()->exec();
    }

    /**
     * uac_permissions TABLE SQL DELETE OPERATION
    ...
    public function delete( $_filters, $_order_by, $offset )
    {
        ...
        bcCache::deleteNamespace( $tbl->tbl_uac_permissions, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->delete( $tbl->tbl_uac_permissions );
    ...
    /**
     * uac_permissions TABLE SQL READ OPERATION
    ...
    public function read( $fetchAll, $_columns, $_filters, $_order_by, $offset )
    {
        ...
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->select( $_columns, $tbl->tbl_uac_permissions );
        bcDB::mapper()->where( $_filters );
        bcDB::mapper()->orderBy( $_order_by );
        bcDB::mapper()->limit( $offset );
    ...
    /**
     * uac_permissions TABLE SQL UPDATE OPERATION
    ...
    public function update( $_columns, $_filters, $_order_by, $offset )
    {
        ...
        bcCache::deleteNamespace( $tbl->tbl_uac_permissions, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->update( $tbl->tbl_uac_permissions, $_columns );
        bcDB::mapper()->where( $_filters );
        bcDB::mapper()->orderBy( $_order_by );
        bcDB::mapper()->limit( $offset );
        return bcDB::mapper()->exec();
    ...
    /**
     * uac_permissions TABLE SQL WRITE OPERATION
    ...
    public function write( $_columns, $last_id )
    {
        ...
        bcCache::deleteNamespace( $tbl->tbl_uac_permissions, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->insert( $tbl->tbl_uac_permissions, $_columns );
