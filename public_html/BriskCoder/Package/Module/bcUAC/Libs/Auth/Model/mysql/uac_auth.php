...
final class uac_auth implements \BriskCoder\Package\Module\bcUAC\Libs\Auth\Model\i_uac_auth
{
    ...
    public function __construct( $CALLER )
    {
        ...
        bcCache::useNamespace( $tbl->tbl_uac_auth );
    }

    /**
     * uac_auth TABLE SQL DELETE OPERATION
    ...
    {
        $tbl = bcUAC::auth()->configModel();
        bcCache::deleteNamespace( $tbl->tbl_uac_auth, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->delete( $tbl->tbl_uac_auth );
    ...
    /**
     * uac_auth TABLE SQL READ OPERATION
    ...
    public function read( $fetchAll, $_columns, $_filters, $_order_by, $offset )
    {
        ...
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->select( $_columns, $tbl->tbl_uac_auth );
        bcDB::mapper()->where( $_filters );
        bcDB::mapper()->orderBy( $_order_by );
        bcDB::mapper()->limit( $offset );
    ...
    /**
     * uac_auth TABLE SQL UPDATE OPERATION
    ...
    public function update( $_columns, $_filters, $_order_by, $offset )
    {
        ...
        bcCache::deleteNamespace( $tbl->tbl_uac_auth, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->update( $tbl->tbl_uac_auth, $_columns );
        bcDB::mapper()->where( $_filters );
        bcDB::mapper()->orderBy( $_order_by );
        bcDB::mapper()->limit( $offset );        
        return bcDB::mapper()->exec();
    ...
    /**
     * uac_auth TABLE SQL WRITE OPERATION
    ...
    public function write( $_columns, $last_id )
    {
        ...
        bcCache::deleteNamespace( $tbl->tbl_uac_auth, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->insert( $tbl->tbl_uac_auth, $_columns );
