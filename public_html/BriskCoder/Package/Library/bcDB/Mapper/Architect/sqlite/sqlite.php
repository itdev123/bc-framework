...
final class sqlite implements i_architect
{
    ...
    public function build( $CLEAR = FALSE )
    {
        ...
        try {
            ...
            //bcDB::mapper()->setFetch()->typeAssociative();
            bcDB::mapper()->show( 'TABLES');
            //bcDB::mapper()->show( 'COLUMNS', 'ds_previews' );
            $_tbl = bcDB::mapper()->fetch( TRUE );
            
            //\BriskCoder\bc::debugger()->breakPoint($tbl[0]);
        ...
        foreach ( $_tbl as $tbl ):
            ...
            try {
                bcDB::mapper()->show( 'COLUMNS', $tbl[0] );
                ...
                foreach ( $_cls as $col ):
                    ...
                    $_columns[$tbl[0]][$col['name']]['type'] = $t;
                    $_columns[$tbl[0]][$col['name']]['size'] = $s;
                    $_columns[$tbl[0]][$col['name']]['unsigned'] = $u;
                    $_columns[$tbl[0]][$col['name']]['null'] = $col['notnull'];
                    $_columns[$tbl[0]][$col['name']]['key'] = $col['pk'];
                    $_columns[$tbl[0]][$col['name']]['default'] = !empty($col['dflt_value']) && ($col['dflt_value'] != 0) ? $col['dflt_value'] : '';
