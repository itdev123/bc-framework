...
use BriskCoder\bc,
    ...
    BriskCoder\Package\Component\bcFormControl\Types\Text\Events,
    BriskCoder\Package\Component\bcFormControl\Types\Text\Validate;
...
final class Text
{
     ...
     * @static $obj
     * @return \BriskCoder\Package\Component\bcFormControl\Types\Text\Events
     */
     ...
     * Requires <!--bc.head--><!--bc--> in the view page to include css and js resource or links
     * @static $obj
     * @return \BriskCoder\Package\Component\bcFormControl\Types\Text\Validate
     */
    ...
    public function getMarkup( $IDENTIFIER, $_ATTRIBUTES, $CUSTOM )
    {
        ...
        if ( !empty( $_validate_settings['VALIDATE_RANGE'] ) ):
            ...
            bc::publisher()->addHeadIncludes( 'bcFormControl.validate.range(\'' . $IDENTIFIER . '\',\'' . $event . '\',\'' . $min . '\',\'' . $max . '\',\'' . $msg_fail . '\', \'' . $msg_pass . '\', \'' . $msg_container . '\');', 'js', FALSE );
        endif;
        if ( !empty( $_validate_settings['VALIDATE_REGEX'] ) ):
            ...
            bc::publisher()->addHeadIncludes( 'bcFormControl.validate.regex(\'' . $IDENTIFIER . '\',\'' . $event . '\',\'' . $mask . '\',\'' . $msg_fail . '\', \'' . $msg_pass . '\', \'' . $msg_container . '\');', 'js', FALSE );
        endif;
        if ( !empty( $_validate_settings['VALIDATE_REQUIRED'] ) ):
            ...
            bc::publisher()->addHeadIncludes( 'bcFormControl.validate.required(\'' . $IDENTIFIER . '\',\'' . $event . '\',\'' . $msg_fail . '\', \'' . $msg_pass . '\', \'' . $msg_container . '\');', 'js', FALSE );
        endif;
        if ( !empty( $_validate_settings['VALIDATE_TYPE'] ) ):
            ...
            bc::publisher()->addHeadIncludes( 'bcFormControl.validate.type(\'' . $IDENTIFIER . '\',\'' . $event . '\',\'' . $type . '\',\'' . $msg_fail . '\', \'' . $msg_pass . '\', \'' . $msg_container . '\');', 'js', FALSE );
        endif;
