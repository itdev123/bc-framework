...
final class Custom
{
    ...
    {
        bcHTML::tag()->div( $MARKUP, array( 'class="bcFormControl-custom-markup"' ) );
        ...
        if ( isset( $this->curr_step[$currNS] ) && !empty( $this->_steps[$currNS][$this->curr_step[$currNS]] ) ):
            ...
            if ( !empty( $this->_steps[$currNS][$this->curr_step[$currNS]]['_message'] ) ):
                if ( isset( $this->curr_message[$currNS] ) && !empty( $this->_steps[$currNS][$this->curr_step[$currNS]]['_message'][$this->curr_message[$currNS]] ) ):
                    bcHTML::tag()->div( $this->_steps[$currNS][$this->curr_step[$currNS]]['_message'][$this->curr_message[$currNS]], array( 'class="bcFormControl-custom-status"' ) );
            ...
            if ( !empty( $buttons ) ):
                bcHTML::tag()->div( $buttons, array( 'class="bcFormControl-custom-submit"' ) );
                $data .= bcHTML::tag()->getMarkup();
            endif;
            if ( $this->_steps[$currNS][$this->curr_step[$currNS]]['form'] === FALSE ):
                $data .= bcHTML::form()->hidden( array( 'name="uri"', 'value="' . $this->_steps[$currNS][$this->curr_step[$currNS]]['uri'] . '"', 'class="bcFormControl-custom-uri"' ) );
            endif;
            ...
            bcHTML::tag()->div( $data, array( 'class="bcFormControl-custom-wrapper"' ) );
            $markup .= bcHTML::tag()->getMarkup();
            bcHTML::tag()->div( $markup, array( 'class="bcFormControl-custom-box"' ) );
            $markup = bcHTML::tag()->getMarkup();
        endif;
        return $markup;
    }
