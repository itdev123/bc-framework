...
final class uac_roles_domains implements \BriskCoder\Package\Module\bcUAC\Libs\RBAC\Model\i_uac_roles_domains
{
    ...
    /**
     * uac_roles_domains TABLE SQL ALTER OPERATION
    ...
    public function alter( $type, $_columns )
    {
        ...
        bcDB::mapper()->alter( $tbl->tbl_uac_roles_domains, $type, $_columns );
        return bcDB::mapper()->exec();
    }

    /**
     * uac_roles_domains TABLE SQL DELETE OPERATION
    ...
    public function delete( $_filters, $_order_by, $offset )
    {
        ...
        bcCache::deleteNamespace( $tbl->tbl_uac_roles_domains, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->delete( $tbl->tbl_uac_roles_domains );
    ...
    /**
     * uac_roles_domains TABLE SQL READ OPERATION
    ...
    public function read( $fetchAll, $_columns, $_filters, $_order_by, $offset )
    {
        ...
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->select( $_columns, $tbl->tbl_uac_roles_domains );
        bcDB::mapper()->where( $_filters );
        bcDB::mapper()->orderBy( $_order_by );
        bcDB::mapper()->limit( $offset );
    ...
    /**
     * uac_roles_domains TABLE SQL UPDATE OPERATION
    ...
    public function update( $_columns, $_filters, $_order_by, $offset )
    {
        ...
        bcCache::deleteNamespace( $tbl->tbl_uac_roles_domains, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->update( $tbl->tbl_uac_roles_domains, $_columns );
        bcDB::mapper()->where( $_filters );
        bcDB::mapper()->orderBy( $_order_by );
        bcDB::mapper()->limit( $offset );
        return bcDB::mapper()->exec();
    ...
    /**
     * uac_roles_domains TABLE SQL WRITE OPERATION
    ...
    public function write( $_columns, $last_id )
    {
        ...
        bcCache::deleteNamespace( $tbl->tbl_uac_roles_domains, TRUE );
        bcDB::connection( $tbl->bcDB_NS );
        bcDB::mapper()->insert( $tbl->tbl_uac_roles_domains, $_columns );
